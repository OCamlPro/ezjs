...
module Attributes = struct
  ...
  let a_aria attr value =
    to_attrib @@ Ezjs_tyxml.Xml.string_attrib ("aria-" ^ attr) value

  let a_role value =
    to_attrib @@ Ezjs_tyxml.Xml.string_attrib "role" value

  let a_data_custom key value =
    to_attrib @@ Ezjs_tyxml.Xml.string_attrib
  ...
  let a_attrib key value =
    to_attrib @@ Ezjs_tyxml.Xml.string_attrib key value
...
module Panel = struct
  ...
  let make_panel
    ...
    let panel_title_content = match panel_title_content with
      | None -> []
      | Some elt ->
        List.iter (Ezjs_tyxml.Manip.addClass elt) (panel_title :: panel_title_class) ;
        [ elt ] in
...
module Menu = struct
  ...
  type menu =
    | Dropdown of string list *
                  Html_types.span_content_fun Ezjs_tyxml.elt list  *
                  menu list * bool
    | Link of string list *
              string * Html_types.flow5_without_interactive Ezjs_tyxml.elt * bool
    | Link2 of string list *
               Html_types.a_attrib attrib list * Html_types.flow5_without_interactive Ezjs_tyxml.Html.elt * bool
    | Action of string list *
                  (unit -> unit) *
                    Html_types.flow5_without_interactive Ezjs_tyxml.elt
    | Separator of string list
    | Header of string list * string
    | Generic of string list * Html_types.li_content_fun Ezjs_tyxml.elt list
