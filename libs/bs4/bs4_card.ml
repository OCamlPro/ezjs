...
and replace_pagination ?classes ?page_sizer id n seps current_page current_size f =
  ...
  Manip.replaceChildren container elts;
  init_popovers ();
        ...
        div ~a:[ a_class [BForm.input_group] ] [
          ...
          div ~a:[ a_class [BForm.input_group_append] ] [
            button ~a:[ a_class [Button.btn; Button.btn_secondary];
                        a_onclick (fun _e ->
                            let popover_input =
                              find_component ("popover-input-" ^ id_pop) in
                            let input_value = Manip.value popover_input in
...
type table_type =
  | Loading of string
  | Ready of (Html_types.tr elt list * string option * int)
...
module MakeCardTable(M: sig
    ...
    val heads : [> Html_types.th_content_fun ] elt list
    val to_row : state option -> t -> [> Html_types.tr ] elt
  ...
  let append_row ?(suf_id="") row =
    ...
    let body_container = find_component id in
    Manip.appendChild body_container row
end
