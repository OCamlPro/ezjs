...
module Utils = struct
  ...
  module Attribute = struct
    let a_attrib key value = to_attrib @@ Xml.string_attrib key value
...
module Items = struct
  ...
  module BCard = struct
    ...
    let make_card
      ...
      let card_title_content = match card_title_content with
        | None -> []
        | Some elt ->
          List.iter (Manip.addClass elt) (card_title :: card_title_class) ;
          [ elt ] in
  ...
  module Progress = struct
    ...
    let update_progress_bar ?text percent id =
      ...
      | None -> log "progress bar %S not found id" id
      | Some bar ->
        Manip.SetCss.width bar (Printf.sprintf "%d%%" percent);
        match text with
        | None -> ()
        | Some text -> Manip.setInnerHtml bar text
  end
  ...
  module Popover = struct
    ...
    let init_popovers () =
      Ezjs_jquery.jQ popover_selector |> Ezjs_jquery.popover_opt |> ignore

    let fill_popover id pop_elt =
      Ezjs_jquery.jQ ("#link-" ^ id) |>
      Ezjs_jquery.on_fun popover_inserted (fun _e ->
          let elt = find_component id in
          Manip.replaceChildren elt pop_elt;
          true) |>
      ignore

    let hide_popovers () =
      Ezjs_jquery.jQ popover_selector |> Ezjs_jquery.popover "hide" |> ignore;
  end
  ...
  module Tooltip = struct
    ...
    let init_tooltips () =
      Ezjs_jquery.jQ tooltip_selector |> Ezjs_jquery.tooltip_opt |> ignore
  end
  ...
  module Toast = struct
    ...
    let show_toast () =
      Ezjs_jquery.jQ ("." ^ toast) |> Ezjs_jquery.toast "show"
  end
