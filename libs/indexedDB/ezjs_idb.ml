...
let wrap ?error ?callback r =
  try
    ...
    r##.onsuccess := AOpt.opt (fun f -> wrap_meth_callback (fun r _e -> f (result r))) callback;
    r##.onerror := AOpt.opt (fun f -> wrap_meth_callback (fun r _e -> f r)) error
  with
  | Js_of_ocaml.Js.Error e ->
    begin match error with
      | Some error -> error (Unsafe.coerce e)
      | None -> js_log e
    end
  | exn -> raise exn
...
let wrapf ?error ?callback g r =
  try
    let r = Lazy.force r in
    r##.onsuccess := AOpt.opt (fun f -> wrap_meth_callback (fun r _e -> f (g @@ result r))) callback;
    r##.onerror := AOpt.opt (fun f -> wrap_meth_callback (fun r _e -> f r)) error
  with
  | Js_of_ocaml.Js.Error e ->
    begin match error with
...
let openDB ?upgrade ?error ?version name callback =
  ...
  (Lazy.force r)##.onupgradeneeded :=
    AOpt.opt (fun u -> wrap_meth_callback (fun r e ->
...
let create_db_options {key_path; auto_increment} : create_db_options t = object%js
  val keyPath = AOpt.opt string key_path
  val autoIncrement = AOpt.opt bool auto_increment
end

let create_store ?options (db : iDBDatabase t) name =
  Unsafe.coerce @@
  db##createObjectStore (string name) (AOpt.opt create_db_options options)

let get_store ?mode (db : iDBDatabase t) name : (_, _) iDBObjectStore t =
  let tr = db##transaction (array [| string name |]) (AOpt.opt str_of_mode mode) in
...
module Store(K : Tr_sig)(D : Tr_sig) = struct
  ...
  let add ?callback ?error ?key (st : store) (x : D.t) =
    wrapf ?callback ?error K.of_js @@ lazy (st##add (D.to_js x) (AOpt.opt K.to_js key))

  let put ?callback ?error ?key (st : store) (x : D.t) =
    wrap ?callback ?error @@ lazy (st##put (D.to_js x) (AOpt.opt K.to_js key))
  ...
  let range ?olower ?oupper ?lower ?upper () =
    ...
    | Some lower, None ->
      KR (iDBKeyRange##lowerBound (K.to_js lower) (AOpt.opt bool olower))
    | None, Some upper ->
      KR (iDBKeyRange##upperBound (K.to_js upper) (AOpt.opt bool oupper))
    | Some lower, Some upper ->
      KR (iDBKeyRange##bound
            (K.to_js lower) (K.to_js upper)
            (AOpt.opt bool olower) (AOpt.opt bool oupper))
  ...
  let iter ?error ?key ?direction (st : (K.js, D.js) iDBObjectStore t)
    ...
    let callback c =
      ...
      | None -> ()
      | Some c ->
        match AOpt.to_opt K.of_js c##.key with
        | Some k ->
          f k (D.of_js c##.value);
    ...
    | None -> wrap ~callback ?error @@
      lazy (st##openCursor AOpt.undefined (AOpt.opt str_of_direction direction))
    | Some (K k) -> wrap ~callback ?error @@
      lazy (st##openCursor (AOpt.def (K.to_js k)) (AOpt.opt str_of_direction direction))
    | Some (KR r) -> wrap ~callback ?error @@
      lazy (st##openCursor_range (AOpt.def r) (AOpt.opt str_of_direction direction))
  ...
  let iter_keys ?error ?key ?direction (st : (K.js, D.js) iDBObjectStore t)
    ...
    match key with
    | None -> wrap ~callback ?error @@
      lazy (st##openKeyCursor AOpt.undefined (AOpt.opt str_of_direction direction))
    | Some (K k) -> wrap ~callback ?error @@
      lazy (st##openKeyCursor (AOpt.def (K.to_js k)) (AOpt.opt str_of_direction direction))
    | Some (KR r) -> wrap ~callback ?error @@
      lazy (st##openKeyCursor_range (AOpt.def r) (AOpt.opt str_of_direction direction))
  ...
  let create_index_options {unique; multi_entry; locale} : create_index_options t = object%js
    val unique = AOpt.opt bool unique
    val multiEntry = AOpt.opt bool multi_entry
    val locale = AOpt.opt bool locale
  end

  let create_index ?options (st : (K.js, D.js) iDBObjectStore t) name key_path =
    let options = AOpt.opt create_index_options options in
