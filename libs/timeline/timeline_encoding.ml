...
let media = conv
  ...
  obj8
    ...
    (opt "caption" string)
    (opt "credit" string)
    (opt "thumbnail" string)
    (opt "alt" string)
    (opt "title" string)
    (opt "link" string)
    (opt "link_target" string)

let slide = conv
    (fun {start_date; end_date; slide_text; media; group; slide_display_date;
          background; autolink; unique_id}
      -> (start_date, end_date, slide_text, media, group, slide_display_date,
          background, autolink, unique_id))
    (fun (start_date, end_date, slide_text, media, group, slide_display_date,
          background, autolink, unique_id)
      -> {start_date; end_date; slide_text; media; group; slide_display_date;
